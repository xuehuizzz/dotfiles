ArangoDB 是一个高效、灵活且易于扩展的多模型数据库，适合用于需要支持多种数据结构并在同一平台上执行复杂查询和事务的应用程序
...
# 导出为jsonl文件
arangoexport --collection students --type jsonl --server.database cmdb --output-directory export_st
udents.jsonl
```
